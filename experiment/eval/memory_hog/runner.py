...
Three chaos types are supported out of the box: ``memory-hog``, ``cpu-hog``
and ``pod-delete``. Train and test phases purposely mix all of them.
...
def _chaos_type_entry(cfg: Dict[str, Any], chaos_type: str) -> Dict[str, Any]:
    ...
    for entry in cfg.get("chaos_types", []) or []:
        if entry.get("name") == chaos_type:
            return entry
    raise KeyError(
        f"chaos_type {chaos_type!r} not declared in config.chaos_types"
    )
...
def build_spec(cfg: Dict[str, Any], service: str, replica_idx: int,
               chaos_type: str) -> Dict[str, Any]:
...
def create_and_start_run(api: ApiClient, cfg: Dict[str, Any],
                         service: str, replica_idx: int, phase: str,
                         chaos_type: str) -> RunRecord:
...
class StrategyResult:
    ...
    correct_fault: Optional[bool]           # WHY:   predicted fault_category == injected category
...
def evaluate_strategy(api: ApiClient, run: RunRecord, strategy: Dict[str, Any],
                      expected_fault: str) -> StrategyResult:
    ...
    *correct* is kept as an alias of *correct_target* for back-compat with
    ...
    try:
        ...
        return StrategyResult(
            ...
            strategy=name,
            predicted_rca=None,
            correct=None, correct_target=None, correct_fault=None, correct_full=None,
            confidence=None,
...
# ── CSV writers ─────────────────────────────────────────────────────────────
RUN_FIELDS = [
    "phase", "chaos_type", "service", "replica_idx", "chaos_target",
    "experiment_id", "run_id", "status", "started_at", "ended_at", "notes",
]
EVAL_FIELDS = [
    "run_id", "chaos_target", "expected_fault_category", "strategy",
    "predicted_rca", "fault_category",
    "correct", "correct_target", "correct_fault", "correct_full",
    "confidence", "prompt_tokens", "neighbours_used",
    ...
    for cell in cells:
        ...
        for rep in range(replicas):
            ...
            if prior is not None:
                print(f"  [{log_prefix}] skip   chaos={chaos_type} service={service} "
                      f"replica={rep} (already completed run_id={prior.run_id})")
                all_records.append(prior)
                _flush_runs(runs_csv, all_records)
                continue
            print(f"  [{log_prefix}] start  chaos={chaos_type} service={service} "
                  f"replica={rep}")
            try:
                ...
                _flush_runs(runs_csv, all_records)
                print(f"  [{log_prefix}] FAIL   chaos={chaos_type} service={service} "
                      f"replica={rep}: {exc}")
                continue
            try:
...
def phase_chaos(
    ...
    if retry_failed:
        ...
        for rec in prior:
            if rec.status == "completed" and rec.run_id:
                completed[(rec.phase, rec.chaos_type, rec.service, rec.replica_idx)] = rec
    ...
    _run_chaos_loop(
        api, cfg, cfg["train_experiments"], int(cfg["replicas"]),
        "train", "TRAIN", completed, all_records, runs_csv,
    )
    print("--- PHASE 2: TEST (held-out runs) ---")
    _run_chaos_loop(
        api, cfg, cfg["test_experiments"], int(cfg["replicas"]),
...
def phase_eval(api: ApiClient, cfg: Dict[str, Any], runs: List[RunRecord],
    ...
    for run in test_runs:
        ...
        if gt_service_persisted and gt_service_persisted != run.chaos_target:
            print(f"  [WARN ] run={run.run_id} chaos_target mismatch: "
...
def _print_summary(results: List[StrategyResult], cfg: Dict[str, Any]) -> None:
    ...
    print(f"{'strategy':<32} {'n':>4} {'where':>8} {'why':>8} {'both':>8} {'avg_tok':>10}")
    ...
    for strat in cfg["strategies"]:
        ...
        print(f"{name:<32} {n:>4} {acc_target:>8.2%} {acc_fault:>8.2%} "
